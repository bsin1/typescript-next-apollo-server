...
> {
  ...
  private getApolloReactHocImport(isTypeImport: boolean): string {
    return `${this.getImportStatement(
      isTypeImport
    )} * as ApolloReactHoc from '${this.config.apolloReactHocImportFrom}';`;
  }
  ...
  private _buildOperationHoc(
    ...
    const propsTypeName: string = this.convertName(nodeName, {
      suffix: "Props",
    });
  ...
  private _buildComponent(
    ...
    const isVariablesRequired =
      operationType === "Query" && hasRequiredVariables(node);
    ...
    const typeToHook = (type: string): string => {
      ...
      switch (type) {
        ...
        case "Query":
          return "useQuery";
        default:
          return `${this.getApolloReactHooksIdentifier()}.use${operationType}`;
      }
    };
